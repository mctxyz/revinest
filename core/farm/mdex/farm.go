...
func NewMdexFarm(farmConfig *config.FarmConfig, client *ethclient.Client, tokenBasic *token.TokenBasic, printer *printer.Printer) *MdexFarm {
	return &MdexFarm{
...
func (c *MdexFarm) Harvest() (*big.Int, error) {
	...
	if pendingReward.Amount.Cmp(big.NewInt(0)) >= 1 {
		...
		if txStatus != 1 {
			return nil, errors.New("Transaction Faild")
		}
